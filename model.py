...
class GAN:
	...
	caption_vector_length : Maxium Length of Caption with padding
	batch_size
	word2vec_dim
	'''
	...
	def generator(self, t_z, t_text_embedding):
		...
		print "Text Embedding", reduced_text_embedding
		z_concat = tf.concat(1, [t_z, reduced_text_embedding])
		print "Noise + Text", z_concat
	...
	def discriminator(self, image, t_text_embedding, reuse=False):
		...
		print "h0", h0
		h1 = ops.lrelu(ops.conv2d(h0, self.options['df_dim']*2, name = 'd_h1_conv')) #16
		print "h1", h1
		h2 = ops.lrelu(ops.conv2d(h1, self.options['df_dim']*4, name = 'd_h2_conv')) #8
		print "h2", h2
		h3 = ops.lrelu(ops.conv2d(h2, self.options['df_dim']*8, name = 'd_h3_conv')) #4
		print "h3", h3
		...
		print "Tiled Embeddings", tiled_embeddings

		h3_concat = tf.concat( 3, [h3, tiled_embeddings], name='h3_concat')
		print "h3_concat", h3_concat

		h4 = ops.linear(tf.reshape(h3_concat, [self.options['batch_size'], -1]), 1, 'd_h3_lin')
		print "h4", h4
